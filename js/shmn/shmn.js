...
btc.decode_adr = function(adr, newpubky)
{
	try
	{
		...
		if(ver == 0x32 && len == 25){ a.type = 'standard'; return a; }
		if(ver == 0x05 && len == 25){ a.type = 'multisig'; return a; }
		
		//if(ver == 0xd4 && len == 37                        ){ a.type = 'wifkey'; a.uncompressed =  true; return a; }
		if(ver == 0xd4 && len == 38 && bytes[len-5] == 0x01){ a.type = 'wifkey'; a.uncompressed = false; return a; }
	}
	catch(e){ }
...
btc.extend = function(pass)
{
	...
	var kiicodeHid = document.getElementById('keyCodeHid').value;
	pass = 'fedda'+pass+'|'+kiicode+'|'+kiicodeHid+'addef';
	
	return pass;
};//___________________________________________________________________________

btc.extend = function(prrivky)
{
	prrivky = SHA256('fedda'+prrivky+'addef');
	
	for(var i = 0; i < 40; i++){ prrivky = prrivky.concat(SHA256(prrivky), RIPEMD160(prrivky)).reverse(); }

	return SHA256(prrivky);
};//___________________________________________________________________________
...
btc.check_entropy2 = function(kycode)
{
	var ent = 0, dic = [];
	
	for(var i = 0; i < kycode.length; i++){ dic[kycode[i]] = (dic[kycode[i]] || 0) + 1; }

	for(var k in dic)
	{
		var p = dic[k] / kycode.length;
...
btc.get_keys = function(prrivky)
{
	var sk = this.extend(prrivky), uncompressed = false; // first, assume regular password

	var adr = this.decode_adr(prrivky);

	if(adr !== false)
	{
		if(adr.type == 'wifkey')
		{
			if(adr.checksum){ sk = this.wif2sk(prrivky); uncompressed = adr.uncompressed; } else return false; // bad WIF key: checksum doesn't match
		}
		else
			if(adr.checksum) return false; // trying to use address as password
	}
